...
L'implementazione di questa classe deve definire 2 metodi principali: name che deve ritornare una stringa con il nome a cui il modello dovrà fare riferimento, e il metodo run che è l'implementazione dell'azione.
...
Per quanto riguarda il NLU dovremo indicare principalmente: il config dove abbiamo definito lingua e strumento di machine learning da usare, dove si trova il dataset e il nome del modello da restituire in output.
...
\iffalse
https://github.com/Wabri/ChatBotPayments#32-backend
ho spiegato il training, il passo dopo è quindi la messa in funzione del server e l'utilizzo nel frontend
\fi
